...
InputT1wFile: Input MRI file, .nii.gz format. Filename must match: sub-<subID>[_ses-<sesID>]_T1w.nii.gz.
InputDWIBase: Base directory that contains these 3 files: <subID>[_ses-<sesID]_dwi.{bval|bvec|nii.gz}.
WorkflowBaseDir: Subject's base directory where statusFile, cse output directory, and nipype workflow directory will be created. Will pass in subject's base directory under the derivatives directory.
PublicDir: Base public directory. Will store files in publicDir/statistics or publicDir/thumbnails. (will attempt to create subdirectories if not exit already)
RunBDP Indicate if workflow should run BDP. {yes, true, y, 1}.
RunSVREG Indicate if workflow should run SVReg. {yes, true, y, 1}.
